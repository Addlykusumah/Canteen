...
const LoginPage = () => {
    ...
    const handleSubmit = async (e: FormEvent) => {
        try {
            e.preventDefault()
            const url = `${BASE_API_URL}/api/login_siswa`
            ...
            if (data.status == true) {
                ...
                if (data.data) {
                    ...
                    if (role === `siswa`) setTimeout(() => router.replace(`/dashboard`), 1000)
                    else if (role === `admin_stan`) setTimeout(() => router.replace(`/cashier/dashboard`), 100)
                }
            }
